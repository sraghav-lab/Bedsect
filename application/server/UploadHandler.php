...
class UploadHandler
{
	...
	'bed_error' => 'This is not a valid bed file',
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'inline_file_types' => '/\.(bed)$/i',
            // Defines which files (based on their names) are accepted for upload:
            'accept_file_types' => '/\.(bed)$/i',
	...
	#$handle = fopen($uploaded_file);
	#while(($fileop = fgetcsv($handle,"\t")) !==false){
	#	if (!preg_match("/chr/",$fileop[0])
	#	|| !preg_match("/[0-9]/",$fileop[1])
	#	|| !preg_match("/[0-9]/",$fileop[2])
	#){
        #      $file->error = $this->get_error_message('bed_error');
        #      return false;
	#}
	#}
	#My bed file validation condition
	#$read_file=fopen($uploaded_file,"r");
	#$read=explode('\t',$read_file);
	#if(count($read)<0){
	#	$file->error = $this->get_error_message('bed_error');
	#	return false;
	#};	
